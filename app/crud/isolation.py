...
def get_aislamiento_by_date_range(db: Session, fecha_inicio: str, fecha_fin: str, skip:int = 0, limit = 10):
    """
    Obtiene las tareas cuya fecha de inicio o fin esté dentro de un rango de fechas.
    Ignora las horas (usa DATE(fecha_hora) y DATE(fecha_hora_fin)).
    """
    try:
        count_query = text("""SELECT COUNT(id_aislamiento) AS total 
                     FROM aislamiento
